...
class GraphConstructor(nn.Module):
    ...
    def _knn_adjacency(self, sim_matrix: torch.Tensor) -> torch.Tensor:
        ...
        row_indices = torch.arange(N, device=sim_matrix.device).unsqueeze(1).expand(-1, k)
        adjacency[row_indices.flatten(), topk_indices.flatten()] = 1.0
        
        # Optionally use similarity weights instead of binary
        # adjacency = adjacency * sim_matrix
        
        return adjacency
    
    def _epsilon_adjacency(self, sim_matrix: torch.Tensor) -> torch.Tensor:
        """Build ε-ball adjacency (connect if similarity > ε)."""
        # Herda dtype da entrada para compatibilidade com float64
